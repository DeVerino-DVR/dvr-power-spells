...
RegisterNetEvent('th_impulsio:prepareImpact', function(impactCoords, spellLevel)
    ...
    SetTimeout(200, function()
        TriggerClientEvent('th_impulsio:applyKnockback', -1, impactCoords, spellLevel, forceUp, forceHorizontal, ragdollTime)
    end)
end)

--- Handle projectile creation event from client to broadcast to all clients
RegisterNetEvent('th_impulsio:createProjectile', function(startCoords, targetCoords, spellLevel)
    local _source = source
    -- Broadcast projectile creation to all clients
    TriggerClientEvent('th_impulsio:createProjectile', -1, startCoords, targetCoords, _source, spellLevel)
end)

--- Handle impact event from client to broadcast EMP particle to all clients
RegisterNetEvent('th_impulsio:onImpact', function(impactCoords, spellLevel)
    -- Broadcast EMP particle effect to all clients
    TriggerClientEvent('th_impulsio:playImpactFx', -1, impactCoords)
end)

local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['th_power']:GetSpell(sourceId, 'impulsio')
    end)
...
local function RegisterImpulsioModule()
    local moduleData = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_impulsio:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_impulsio:prepareCast', source, spellLevel)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_impulsio] Module enregistré avec succès')
end
