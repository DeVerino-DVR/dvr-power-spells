            ...
            if type(data) == "table" and DoesEntityExist(data.prop) then
                if currentTime < data.endTime then
                    ...
                    if data.targetId and data.targetId > 0 then
                        TriggerServerEvent('dvr_sufferis:ragdollTarget', data.targetId, data.level or 0)
                    end
...
RegisterNetEvent('dvr_sufferis:applyRagdoll', function(level)
...
RegisterNetEvent('dvr_sufferis:prepareProjectile', function(targetId, level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_sufferis:broadcastProjectile', finalTargetCoords, targetId or 0, level or 0)
        Wait(800)
        StopWandTrail(casterPed)
    end)
end)

RegisterNetEvent('dvr_sufferis:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_sufferis:spawnProjectile', function(sourceServerId, targetCoords, targetId, level)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then return end
