...
RegisterNetEvent('dvr_fumora:cast', function(coords)
    ...
    TriggerClientEvent('dvr_fumora:spawn', -1, screenId, { x = x, y = y, z = z }, duration)
end)
...
local function RegisterFumoraModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, _, sourceId, _, level)
            ...
            TriggerClientEvent('dvr_fumora:otherPlayerCasting', -1, sourceId)
            TriggerClientEvent('dvr_fumora:prepare', sourceId)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_fumora] Module enregistré avec succès')
end
