...
AddEventHandler('th_hiddenis:syncHidden', function(serverId, hidden)
    if hidden then
...
local function RevealPlayer(targetServerId, casterServerId)
    ...
    TriggerClientEvent('th_exposare:forceReveal', targetServerId)
    
    Notify(targetServerId, Config.Messages.revealed)
    
    TriggerClientEvent('th_exposare:showRevealEffect', -1, targetServerId)
...
local function RegisterExposareModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_exposare:areaReveal', -1, casterCoords, radius)
    ...
    print('[th_exposare] Module enregistré avec succès')
end
