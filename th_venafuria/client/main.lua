                ...
                if currentTime >= data.endTime then
                    if DoesEntityExist(data.prop) then
                        ...
                        if isLocalCaster then
                            TriggerServerEvent('th_venafuria:applyEffect', propCoords, data.targetId, data.level)
                        end
                        ...
                        if currentPos.z <= levData.startZ + 0.5 or fallElapsed > 800 then
                            if levData.isFinalSmash then
                                ...
                                if isLocalTarget and not levData.bloodSpawned then
                                    ...
                                    TriggerServerEvent('th_venafuria:bloodImpact', impactCoords)
                                end
...
RegisterNetEvent('th_venafuria:startLevitation', function(level, casterId)
...
RegisterNetEvent('th_venafuria:syncLevitation', function(targetId, level, casterId)
    local myServerId = GetPlayerServerId(PlayerId())
...
RegisterNetEvent('th_venafuria:dropTarget', function()
    local myServerId = GetPlayerServerId(PlayerId())
...
RegisterNetEvent('th_venafuria:syncDrop', function(targetId)
...
RegisterNetEvent('th_venafuria:syncSmash', function(targetId, smashNumber, totalSmashes)
    local levData = activeLevitations[targetId]
...
RegisterNetEvent('th_venafuria:syncBloodImpact', function(coords)
...
RegisterNetEvent('th_venafuria:prepareProjectile', function(targetId, level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('th_venafuria:broadcastProjectile', finalTargetCoords, targetId or 0, level or 0)
    end)
    ...
    CreateThread(function()
        ...
        for i, timing in ipairs(smashTimings) do
            ...
            TriggerServerEvent('th_venafuria:triggerSmash', targetId or 0, i, #smashTimings)
        end
    end)
end)

RegisterNetEvent('th_venafuria:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('th_venafuria:spawnProjectile', function(sourceServerId, targetCoords, targetId, level)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then return end
