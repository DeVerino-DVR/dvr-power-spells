                ...
                if currentTime >= data.endTime then
                    if data.projectileType == 1 then
                        ...
                        if data.sourceServerId == myServerId and data.targetServerId and data.targetServerId > 0 then
                            TriggerServerEvent('dvr_cruorax:applyEffect', data.targetServerId, data.spellLevel)
                        end
...
RegisterNetEvent('dvr_cruorax:prepareProjectile', function(spellLevel)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_cruorax:broadcastProjectile', finalTargetCoords, spellLevel, targetServerId, 1)
        ...
        TriggerServerEvent('dvr_cruorax:playSecondAnimation')
        ...
        TriggerServerEvent('dvr_cruorax:broadcastProjectile', finalTargetCoords, spellLevel, targetServerId, 2)
...
RegisterNetEvent('dvr_cruorax:otherPlayerCasting', function(sourceServerId)
    local myServerId = GetPlayerServerId(PlayerId())
...
RegisterNetEvent('dvr_cruorax:playSecondAnim', function()
    local casterPed = cache.ped
...
RegisterNetEvent('dvr_cruorax:otherPlayerSecondAnim', function(sourceServerId)
    local myServerId = GetPlayerServerId(PlayerId())
...
RegisterNetEvent('dvr_cruorax:fireProjectile', function(sourceServerId, targetCoords, targetServerId, level, duration, projectileType)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
...
RegisterNetEvent('dvr_cruorax:applySurrender', function(duration, level)
    ...
    if not hasShield then
        local ok, result = pcall(function()
            return exports['dvr_prothea'] and exports['dvr_prothea'].hasLocalShield and exports['dvr_prothea']:hasLocalShield()
        end)
...
RegisterNetEvent('dvr_cruorax:removeSurrender', function()
    local playerPed = cache.ped
    if LocalPlayer and LocalPlayer.state then
