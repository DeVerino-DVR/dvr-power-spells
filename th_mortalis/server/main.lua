...
RegisterNetEvent('th_mortalis:broadcastProjectile', function(finalTargetCoords)
    ...
    if GetResourceState('th_power') == 'started' then
        clamped = exports['th_power']:ClampSpellCoords(_source, 'mortalis', coords, nil, false) or coords
    end
    
    TriggerClientEvent('th_mortalis:fireProjectile', -1, _source, clamped)
end)
        ...
        onCast = function(hasItem, raycast, source, target)
            ...
            SetTimeout(5000, function()
                local ok, applied = pcall(function()
                    return exports['th_power']:DealDamage(target, 100, 100, 'mortalis')
                end)
            end)
            exports['th_power']:LogSpellCast(data)
            
            TriggerClientEvent('th_mortalis:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_mortalis:prepareProjectile', source)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_mortalis] Module enregistré avec succès')
end
