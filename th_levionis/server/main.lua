...
local function RegisterLevionisModule()
    ...
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            if target and target ~= -1 then
                ...
                TriggerClientEvent('th_levionis:startLevitation', -1, target, source, desiredDuration, levCfg.height or 1.5, targetCoords, spellLevel)
            else
                TriggerClientEvent('th_levionis:startLevitation', -1, -1, source, desiredDuration, levCfg.height or 1.5, targetCoords, spellLevel)
            end
    ...
    print('[th_levionis] Module enregistré avec succès')
end
