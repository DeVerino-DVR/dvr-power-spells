                            ...
                            if data.targetEntity and DoesEntityExist(data.targetEntity) then
                                if IsEntityAPed(data.targetEntity) then
                                    if IsPedAPlayer(data.targetEntity) then
                                        ...
                                        TriggerServerEvent('dvr_oublitix:applySpell', targetServerId, data.level)
                                    end
                                end
                            end
...
RegisterNetEvent('dvr_oublitix:prepareProjectile', function()
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_oublitix:broadcastProjectile', finalTargetCoords, targetEntity)
...
RegisterNetEvent('dvr_oublitix:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_oublitix:fireProjectile', function(sourceServerId, targetCoords, targetEntity, level)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then 
        return 
    end
...
RegisterNetEvent('dvr_oublitix:applyEffect', function(rollbackMinutes, targetCoords)
