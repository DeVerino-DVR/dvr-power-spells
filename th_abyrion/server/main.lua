...
RegisterNetEvent('th_abyrion:cast', function(coords, level)
    ...
    if lvl == 5 and playerCircles[_source] then
        ...
        TriggerClientEvent('th_abyrion:destroyCircle', -1, existingCircleId, true)
    ...
    TriggerClientEvent('th_abyrion:spawnCircle', -1, _source, coords, lvl, circleId)

    if lvl < 5 then
        SetTimeout(levelConfig.duration, function()
            activeCircles[circleId] = nil
            TriggerClientEvent('th_abyrion:destroyCircle', -1, circleId)
        end)
    end
end)

RegisterNetEvent('th_abyrion:playerBurning', function(targetServerId, duration)
    TriggerClientEvent('th_abyrion:showBurningFx', -1, targetServerId, duration)
end)
...
local function RegisterAbyrionModule()
    ...
    local moduleData = {
        ...
        sound = nil,
        soundType = nil,
        image = Config.Module.image or "images/power/th_abyrion.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            if not isToggleOff then
                TriggerClientEvent('th_abyrion:otherPlayerCasting', -1, source)
            end

            TriggerClientEvent('th_abyrion:prepare', source, spellLevel, isToggleOff)
    ...
    print('[th_abyrion] Module enregistre avec succes')
end
