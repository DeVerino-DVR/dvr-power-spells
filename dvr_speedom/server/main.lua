...
RegisterNetEvent('dvr_speedom:applySpeed', function(targetServerId)
    local _source <const> = source
    local target = targetServerId and targetServerId > 0 and targetServerId or _source
    TriggerClientEvent('dvr_speedom:grantSpeed', target, Config.Buff.duration or 30000, _source ~= target)
end)
...
local function RegisterSpeedomModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_speedom:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_speedom:prepareSpeed', source, target or 0)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_speedom] Module enregistré avec succès')
end
