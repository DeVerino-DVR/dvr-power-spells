...
RegisterNetEvent('dvr_ignifera:broadcastProjectile', function(finalTargetCoords, spellLevel)
    local _source <const> = source
    if not finalTargetCoords then
        return
    end
    
    TriggerClientEvent('dvr_ignifera:fireProjectile', -1, _source, finalTargetCoords, spellLevel)
end)

RegisterNetEvent('dvr_ignifera:applyExplosionDamage', function(explosionCoords, spellLevel)
    local _source <const> = source
...
local function RegisterIgniferaModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_ignifera:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_ignifera:prepareProjectile', source, spellLevel)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_ignifera] Module enregistré avec succès')
end
