    ...
    if DoesEntityExist(propHandle) then
        ...
        if DoesEntityExist(propHandle) then
            ...
            if netId and netId > 0 then
                TriggerServerEvent('dvr_prothea:forceDeleteEntity', netId)
            end
...
local function IsProtheaUnlocked()
    ...
    local ok, hasSpell = pcall(function()
        local unlocked = exports['dvr_power']:GetSpell(SPELL_ID)
        return unlocked
    end)
...
local function TryCastShield()
    ...
    TriggerServerEvent('dvr_prothea:castShield')
end
...
RegisterNetEvent('dvr_prothea:applyShield', function(targetServerId, duration, blockDamage, applyGodmode, spawnProps, spellLevel)
...
RegisterNetEvent('dvr_prothea:forceDeleteEntityClient', function(netId)
...
-- Staturion petrify
RegisterNetEvent('dvr_staturion:applyPetrify', function()
    if ShouldNegateIncomingSpell() then
        BlockedSpellEvent('dvr_staturion:applyPetrify', 'petrify annulé')
        CancelEvent()
    end
end)

-- Repulsar knockback
RegisterNetEvent('dvr_repulsar:receiveForce', function()
    if ShouldNegateIncomingSpell() then
        BlockedSpellEvent('dvr_repulsar:receiveForce', 'repoussé ignoré')
        CancelEvent()
    end
end)

-- Levionis levitation
RegisterNetEvent('dvr_levionis:startLevitation', function(targetServerId)
    if ShouldNegateIncomingSpell() and targetServerId == GetPlayerServerId(PlayerId()) then
        BlockedSpellEvent('dvr_levionis:startLevitation', 'lévitation annulée')
        CancelEvent()
    end
end)

-- Levionis fall/stop cleanup events ignored while shielded
RegisterNetEvent('dvr_levionis:stopLevitation', function(targetServerId)
    if ShouldNegateIncomingSpell() and targetServerId == GetPlayerServerId(PlayerId()) then
        BlockedSpellEvent('dvr_levionis:stopLevitation', 'stop levitation ignoré')
        CancelEvent()
    end
end)

-- Animarion forced transformation
RegisterNetEvent('dvr_animarion:applyTransform', function()
    if ShouldNegateIncomingSpell() then
        BlockedSpellEvent('dvr_animarion:applyTransform', 'transformation annulée')
        CancelEvent()
    end
end)

-- Accyra projectile visuals hitting player (ignore if shielded)
RegisterNetEvent('dvr_accyra:fireProjectile', function(sourceServerId)
    local myServerId <const> = GetPlayerServerId(PlayerId())
    if sourceServerId ~= myServerId and ShouldNegateIncomingSpell() then
        BlockedSpellEvent('dvr_accyra:fireProjectile', 'projectile ignoré')
        CancelEvent()
    end
end)

RegisterNetEvent('dvr_prothea:castResult', function(status, data)
    if status == 'ok' then
...
RegisterNetEvent('dvr_prothea:removeShield', function(targetServerId)
