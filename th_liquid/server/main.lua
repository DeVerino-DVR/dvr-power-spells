...
local function SetLiquidState(sourceId, enabled, level)
    ...
    if not enabled then
        ...
        print(('[th_liquid] disable | src=%s'):format(sourceId))
        TriggerClientEvent('th_liquid:apply', -1, sourceId, false, 0, level or 1)
        return
    end
    ...
    print(('[th_liquid] enable | src=%s level=%s duration=%s'):format(sourceId, level or 1, duration))
    TriggerClientEvent('th_liquid:apply', -1, sourceId, true, duration, level or 1)
    ...
    if not infinite then
        SetTimeout(duration, function()
            ...
            if state and state.expires <= (os_time() * 1000) then
                activePlayers[sourceId] = nil
                TriggerClientEvent('th_liquid:apply', -1, sourceId, false, duration, level or 1)
            end
        end)
    end
end
...
RegisterNetEvent('th_liquid:burst', function(coords, targetNetId)
    ...
    TriggerClientEvent('th_liquid:burstFx', -1, normalized, targetNetId or 0)
end)

RegisterNetEvent('th_liquid:toggle', function(desiredState)
    local sourceId = tonumber(source)
    if not sourceId then
        return
    end
...
AddEventHandler('playerDropped', function()
    ...
    if activePlayers[sourceId] then
        activePlayers[sourceId] = nil
        TriggerClientEvent('th_liquid:apply', -1, sourceId, false, 0, 1)
    end
...
local function RegisterLiquidModule()
    local moduleData <const> = {
        ...
        isBasic = false,
        image = Config.Module.image or 'images/power/th_liquid.png',
        ...
        onCast = function(hasItem, _, sourceId, _, level)
            ...
            print(('[th_liquid] onCast | src=%s level=%s'):format(sourceId, level or 1))
    ...
    print('[th_liquid] Module enregistré avec succès')
end
