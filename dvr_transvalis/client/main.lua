...
local function GetTransvalisConfig()
    if exports['dvr_transvalis'] and exports['dvr_transvalis'].GetTransvalisConfig then
        return exports['dvr_transvalis']:GetTransvalisConfig()
    end
    ...
    CreateThread(function()
        ...
        if config.bringPlayers and #playersToReturn > 0 then
            ...
            if #returnTargets > 0 then
                TriggerServerEvent('dvr_transvalis:teleportMultiplePlayers', returnTargets, returnPos)
            end
        end
...
local function BringNearbyPlayers(radius)
    ...
    for _, target in ipairs(targets) do
        TriggerServerEvent('dvr_transvalis:teleportPlayerToMe', target.serverId, myCoords)
    end
...
local function StopTransplanning(sync)
    ...
    if sync ~= false then
        TriggerServerEvent('dvr_transvalis:syncStop')
    end
end
            ...
            if remaining <= 0 then
                ...
                if isMoving then
                    ...
                    if currentTime - lastSyncTime >= syncTickRate then
                        ...
                        local myServerId = GetPlayerServerId(PlayerId())
                        TriggerServerEvent('dvr_transvalis:syncPosition', myServerId, {
                            x = targetCoords.x,
                            y = targetCoords.y,
                            z = targetCoords.z
                        }, {
...
RegisterNetEvent('dvr_transvalis:start', function(sourceId, duration, trailColor)
...
RegisterNetEvent('dvr_transvalis:stop', function(sourceId, coords)
    if not sourceId then
        return
    end
...
RegisterNetEvent('dvr_transvalis:arrivalEffects', function(sourceId, coords)
...
RegisterNetEvent('dvr_transvalis:syncPositionClient', function(sourceId, coords, fromCoords, trailColor, isMoving)
    if not sourceId or not coords then
        return
    end
...
RegisterNetEvent('dvr_transvalis:cancelRequested', function()
    if isTransplanning then
        StopTransplanning()
    end
end)

RegisterNetEvent('dvr_transvalis:clientTeleport', function(coords)
...
RegisterNetEvent('dvr_transvalis:saveReturnPosition')
AddEventHandler('dvr_transvalis:saveReturnPosition', SaveReturnPosition)

RegisterNetEvent('dvr_transvalis:bringNearbyPlayers')
AddEventHandler('dvr_transvalis:bringNearbyPlayers', BringNearbyPlayers)

RegisterNetEvent('dvr_transvalis:returnTeleportedPlayers')
AddEventHandler('dvr_transvalis:returnTeleportedPlayers', ReturnTeleportedPlayers)
