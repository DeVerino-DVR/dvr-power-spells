local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['dvr_power']:GetSpell(sourceId, 'transvalis')
    end)
...
local function RegisterTransplannerModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_transvalis:start', -1, source, duration, trailColor)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_transvalis] Module enregistré avec succès')
end
...
RegisterNetEvent('dvr_transvalis:syncStop', function()
    local src <const> = source
    TriggerClientEvent('dvr_transvalis:stop', -1, src)
end)

-- Event pour synchroniser la position du transplanage en temps réel
RegisterNetEvent('dvr_transvalis:syncPosition', function(sourceId, coords, fromCoords, trailColor, isMoving)
    local src = source
    ...
    TriggerClientEvent('dvr_transvalis:syncPositionClient', -1, sourceId, coords, validFromCoords, trailColor, isMoving)
end)

RegisterNetEvent('dvr_transvalis:teleportPlayer', function(targetServerId, coords)
    local src = source
    ...
    TriggerClientEvent('dvr_transvalis:clientTeleport', targetServerId, coords)
end)

RegisterNetEvent('dvr_transvalis:teleportSinglePlayer', function(targetServerId, coords)
    local src = source
...
RegisterNetEvent('dvr_transvalis:teleportPlayerToMe', function(targetServerId, myCoords)
    local src = source
    ...
    TriggerClientEvent('dvr_transvalis:clientTeleport', targetServerId, myCoords)
end)

RegisterNetEvent('dvr_transvalis:startMenuTeleport', function(sourceId, duration, trailColor)
    local src = source
    ...
    TriggerClientEvent('dvr_transvalis:start', -1, sourceId, duration, trailColor)
end)

RegisterNetEvent('dvr_transvalis:teleportMultiplePlayers', function(targets, destinationCoords)
    local src = source
    ...
    for _, target in ipairs(targets) do
        if target and target.serverId then
            ...
            if targetServerId and targetServerId ~= src and playerName then
                ...
                TriggerClientEvent('dvr_transvalis:clientTeleport', targetServerId, {
                    x = offsetX,
                    y = offsetY,
                    z = offsetZ
                })
...
RegisterNetEvent('dvr_transvalis:broadcastStart', function(sourceId, duration, trailColor)
    local src = source
    ...
    TriggerClientEvent('dvr_transvalis:start', -1, sourceId, duration, actualTrailColor)
end)

RegisterNetEvent('dvr_transvalis:broadcastArrivalEffects', function(sourceId, coords)
    local src = source
    ...
    TriggerClientEvent('dvr_transvalis:arrivalEffects', -1, sourceId, coords)
end)

RegisterNetEvent('dvr_transvalis:stopMenuTeleport', function(sourceId)
    local src = source
    ...
    TriggerClientEvent('dvr_transvalis:stop', -1, sourceId)
end)
