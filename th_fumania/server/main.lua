...
RegisterNetEvent('th_fumania:swapPositions', function(targetServerId, sourceCoords, targetCoords)
    ...
    TriggerClientEvent('th_fumania:doSwap', -1, _source, targetServerId, sourceCoords, targetCoords)
end)
...
local function RegisterFumaniaModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_fumania:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_fumania:prepareSwap', source)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_fumania] Module enregistré avec succès')
end
