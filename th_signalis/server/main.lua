...
RegisterNetEvent('th_signalis:broadcastProjectile', function(finalTargetCoords)
    local _source <const> = source
    if not finalTargetCoords then
        return
    end
    
    TriggerClientEvent('th_signalis:fireProjectile', -1, _source, finalTargetCoords)
end)
...
local function RegisterSignalisModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_signalis:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_signalis:prepareProjectile', source)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_signalis] Module enregistré avec succès')
end
