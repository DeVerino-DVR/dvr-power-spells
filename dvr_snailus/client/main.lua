...
RegisterNetEvent('dvr_snailus:sync', function(serverId, enabled, durationMs, speedMultiplier)
        ...
        if isLocalSlowed then
            ...
            if localSlowEndTime and localSlowEndTime ~= -1 then
                ...
                if now >= localSlowEndTime then
                    local localServerId = GetPlayerServerId(PlayerId())
                    TriggerServerEvent('dvr_snailus:timeExpired', localServerId)
                    RemoveSlowEffect()
                end
            end
        else
...
CreateThread(function()
    Wait(2500)
    TriggerServerEvent('dvr_snailus:requestSync')
end)
