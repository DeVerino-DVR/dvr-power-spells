                            ...
                            if data.targetEntity and DoesEntityExist(data.targetEntity) then
                                if IsEntityAPed(data.targetEntity) then
                                    if IsPedAPlayer(data.targetEntity) then
                                        ...
                                        TriggerServerEvent('dvr_staturion:petrifyPlayer', targetServerId)
                                    else
                                        TriggerEvent('dvr_staturion:petrifyPed', data.targetEntity, data.petrifyDuration)
                                    end
                                end
                            end
...
RegisterNetEvent('dvr_staturion:prepareProjectile', function()
...
RegisterNetEvent('dvr_staturion:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_staturion:fireProjectile', function(sourceServerId, targetCoords, targetEntity, level, duration)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then 
        return 
    end
...
RegisterNetEvent('dvr_staturion:applyPetrify', function(duration, level)
...
RegisterNetEvent('dvr_staturion:removePetrify', function()
    local playerPed = cache.ped
    if LocalPlayer and LocalPlayer.state then
...
RegisterNetEvent('dvr_staturion:petrifyPed', function(pedEntity, duration)
