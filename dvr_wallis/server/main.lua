...
RegisterNetEvent('dvr_wallis:broadcastWall', function(targetCoords, direction, spellLevel)
    ...
    TriggerClientEvent('dvr_wallis:spawnWall', -1, _source, targetCoords, direction, level)

    if Config.Debug then
        print(string.format('[dvr_wallis] Wall created - Level %d - Coords: %.2f, %.2f, %.2f',
            level, targetCoords.x, targetCoords.y, targetCoords.z))
    end
end)

--- Register the Wallis module with dvr_power
local function RegisterWallisModule()
    ...
    local moduleData <const> = {
        ...
        soundType = "3d",
        image = Config.Module.image or "images/power/dvr_wallis.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_wallis:otherPlayerCasting', -1, source)

            -- Start cast sequence on caster's client
            TriggerClientEvent('dvr_wallis:prepareCast', source, spellLevel)
    ...
    print('[dvr_wallis] Module enregistré avec succès')
end
