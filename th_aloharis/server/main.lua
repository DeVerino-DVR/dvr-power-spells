...
local function registerModule()
    ...
    local moduleData <const> = {
        ...
        onCast = function(hasWand, _, source, _, spellLevel)
            ...
            TriggerClientEvent('th_aloharis:onCast', source, level)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_aloharis] Module enregistré avec succès')
end
...
RegisterNetEvent('th_aloharis:unlockDoor', function(payload)
    local src = source
    ...
    if doorId == nil then
        TriggerClientEvent('th_aloharis:doorUnlocked', src, { success = false, source = src, reason = 'failed' })
        return
    end

    local doorName = payload.name
    if doorIsBlacklisted(doorId, doorName) then
        TriggerClientEvent('th_aloharis:doorUnlocked', src, { success = false, source = src, reason = 'blacklisted' })
        return
    end

    if payload.state == 0 then
        TriggerClientEvent('th_aloharis:doorUnlocked', src, { success = false, source = src, reason = 'alreadyUnlocked' })
        return
    end
    ...
    TriggerClientEvent('th_aloharis:doorUnlocked', -1, {
        success = true,
        source = src,
        id = numericId,
        coords = coords,
    ...
    TriggerEvent('th_aloharis:doorUnlockedServer', src, numericId, coords)
end)
