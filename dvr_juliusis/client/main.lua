...
local function CleanupProjectile(propId)
    local data = juliusisProjectiles[propId]
    if not data then
        return
    end
    ...
    juliusisProjectiles[propId] = nil
end

local function CreateJuliusisProjectile(startCoords, targetCoords, sourceServerId, casterPed, targetId, level)
...
CreateThread(function()
    while true do
        ...
        for propId, data in pairs(juliusisProjectiles) do
            if type(data) == "table" then
                ...
                if currentTime >= data.endTime then
                    if DoesEntityExist(data.prop) then
                        ...
                        TriggerServerEvent('dvr_juliusis:ragdollTarget', data.targetId or 0, data.level or 0)
...
local function GetRagdollDuration(level)
    local base = Config.Ragdoll.baseDuration or 3000
    local perLevel = Config.Ragdoll.perLevel or 0
    local max = Config.Ragdoll.maxDuration or 6000
    local lvl = tonumber(level) or 0
    local duration = base + (perLevel * lvl)
    return math.min(duration, max)
end

RegisterNetEvent('dvr_juliusis:playSound', function(sourceServerId, casterCoords)
    if not casterCoords then
        return
    end

    local soundUrl = 'YOUR_SOUND_URL_HERE'

    -- REPLACE WITH YOUR SOUND SYSTEM
    -- exports['lo_audio']:playSound({
    -- id = ('juliusis_cast_%s'):format(GetGameTimer()),
    -- url = soundUrl,
    -- volume = 0.4,
    -- loop = false,
    -- spatial = true,
...
RegisterNetEvent('dvr_juliusis:applyRagdoll', function(level)
    if HasProtheaShield() then
        print('[Juliusis] Ragdoll ignoré (bouclier Prothea actif)')
        return
    end
...
RegisterNetEvent('dvr_juliusis:prepareProjectile', function(targetId, level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_juliusis:broadcastProjectile', finalTargetCoords, targetId or 0, level or 0)
...
RegisterNetEvent('dvr_juliusis:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_juliusis:spawnProjectile', function(sourceServerId, targetCoords, targetId, level)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
    ...
    local handBone <const> = GetPedBoneIndex(casterPed, 28422)
    local startCoords <const> = GetWorldPositionOfEntityBone(casterPed, handBone)
    CreateJuliusisProjectile(startCoords, targetCoords, sourceServerId, casterPed, targetId or 0, level or 0)
end)
