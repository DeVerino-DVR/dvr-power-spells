...
local function CreateAvadaProjectile(startCoords, targetCoords, sourceServerId)
...
CreateThread(function()
    while true do
        local hasProjectiles = next(avadaRayProps) ~= nil
        if not hasProjectiles then
            Wait(200)
        else
            local currentTime <const> = GetGameTimer()
            
            for propId, data in pairs(avadaRayProps) do
...
RegisterNetEvent('dvr_mortalis:prepareProjectile', function()
    ...
    CreateThread(function()
        ...
        local adjustedDelay = delay / math.max(speedMult * 0.6, 0.1) + 50 -- allonge un peu le départ pour coller à la fin du geste
        ...
        TriggerServerEvent('dvr_mortalis:broadcastProjectile', finalTargetCoords)
...
RegisterNetEvent('dvr_mortalis:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_mortalis:fireProjectile', function(sourceServerId, targetCoords)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then 
        return 
    end
    ...
    CreateAvadaProjectile(startCoords, targetCoords, sourceServerId)
end)
