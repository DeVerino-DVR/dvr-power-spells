    ...
    local moduleData <const> = {
        ...
        onCast = function(hasWand, _, source, target, spellLevel)
            ...
            if exports['dvr_prothea'] and exports['dvr_prothea'].hasActiveShield and exports['dvr_prothea']:hasActiveShield(target) then
                print(string.format('[dvr_desarmis] annule par Prothea pour %s', target))
                return false
            end
            ...
            TriggerClientEvent('dvr_desarmis:onCast', source, target, spellLevelNum)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_desarmis] Module enregistré avec succès')
end

RegisterNetEvent('dvr_desarmis:disarmTarget', function(targetId)
    local src = source
    ...
    if exports['dvr_prothea'] and exports['dvr_prothea'].hasActiveShield and exports['dvr_prothea']:hasActiveShield(targetId) then
        print(string.format('[dvr_desarmis] disarm annule par Prothea pour %s', targetId))
        if Config.Messages and Config.Messages.failed then
            lib.notify(src, Config.Messages.failed)
        end
        return
    ...
    TriggerClientEvent('dvr_desarmis:disarmed', targetId)
    TriggerClientEvent('dvr_desarmis:playImpact', -1, targetId)
end)
