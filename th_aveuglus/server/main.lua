...
local function SyncPlayerState(target, serverId, enabled, durationMs)
    TriggerClientEvent('th_aveuglus:sync', target, serverId, enabled, durationMs)
end

RegisterNetEvent('th_aveuglus:requestSync', function()
...
RegisterNetEvent('th_aveuglus:timeExpired', function(serverId)
...
local function RegisterAveuglusModule()
    ...
    local moduleData <const> = {
        ...
        isBasic = false,
        selfCast = false,
        image = Config.Module.image or 'images/power/th_aveuglus.png',
    ...
    print('[th_aveuglus] Module enregistré avec succès')
end
