...
RegisterNetEvent('th_flashstep:dash', function(targetCoords)
    local _source <const> = source
    if not targetCoords then
        return
    end
    TriggerClientEvent('th_flashstep:doDash', -1, _source, targetCoords)
end)
...
local function RegisterFlashstepModule()
    local moduleData <const> = {
        ...
        soundType = "3d",
        image = Config.Module.image or "images/power/th_flashstep.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_flashstep:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_flashstep:prepareDash', source)
            return true
        end
    }
    exports['th_power']:registerModule(moduleData, 0)
    print('[th_flashstep] Module enregistré avec succès')
end
