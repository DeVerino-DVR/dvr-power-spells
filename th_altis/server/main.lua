...
RegisterNetEvent('th_altis:applyJump', function(targetServerId, spellLevel)
    ...
    if not level then
        ...
        local hasSpell, levelFromExport = exports['th_power']:GetSpell(_source, 'altis')
        level = levelFromExport or 0
    end
    local duration = GetDurationForLevel(level)
    
    TriggerClientEvent('th_altis:grantJump', target, duration, _source ~= target)
end)
...
local function RegisterAltisModule()
    local moduleData <const> = {
        ...
        sound = '',
        soundType = "3d",
        image = Config.Module.image or "images/power/th_altis.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            if spellLevel == 0 then
                local hasSpell, levelFromExport = exports['th_power']:GetSpell(source, 'altis')
            ...
            TriggerClientEvent('th_altis:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_altis:prepareJump', source, target or 0, spellLevel)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_altis] Module enregistré avec succès')
end
