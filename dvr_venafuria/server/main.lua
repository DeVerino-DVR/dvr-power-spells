...
local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['dvr_power']:GetSpell(sourceId, 'venafuria')
    end)
...
RegisterNetEvent('dvr_venafuria:broadcastProjectile', function(targetCoords, targetId, level)
    local _source <const> = source
    if not targetCoords then return end
    TriggerClientEvent('dvr_venafuria:spawnProjectile', -1, _source, targetCoords, targetId or 0, level or 0)
end)

RegisterNetEvent('dvr_venafuria:applyEffect', function(impactCoords, targetId, level)
    local _source <const> = source
...
RegisterNetEvent('dvr_venafuria:triggerDrop', function(targetId)
    local _source <const> = source
    if not targetId or targetId <= 0 then return end
    TriggerClientEvent('dvr_venafuria:syncDrop', -1, targetId)
end)

RegisterNetEvent('dvr_venafuria:triggerSmash', function(targetId, smashNumber, totalSmashes)
    local _source <const> = source
    if not targetId or targetId <= 0 then return end
    -- Broadcast le smash à tous les clients
    TriggerClientEvent('dvr_venafuria:syncSmash', -1, targetId, smashNumber, totalSmashes)
end)

RegisterNetEvent('dvr_venafuria:bloodImpact', function(coords)
    local _source <const> = source
    if not coords then return end
    -- Broadcast l'explosion de sang à tous les clients
    TriggerClientEvent('dvr_venafuria:syncBloodImpact', -1, coords)
end)
...
local function RegisterVenaFuriaModule()
    ...
    local moduleData <const> = {
        ...
        soundType = "3d",
        image = Config.Module.image or "images/power/dvr_venafuria.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_venafuria:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_venafuria:prepareProjectile', source, targetId, spellLevel)

            -- Lever la cible immédiatement dès le lancer du sort (broadcast à tous pour sync)
            if not (exports['dvr_prothea'] and exports['dvr_prothea'].hasActiveShield and exports['dvr_prothea']:hasActiveShield(targetId)) then
                TriggerClientEvent('dvr_venafuria:syncLevitation', -1, targetId, spellLevel, source)
            end
    ...
    print('[dvr_venafuria] Module enregistre avec succes')
end
