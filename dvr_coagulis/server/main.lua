...
local function RegisterCoagulisModule()
    local moduleData = {
        ...
        soundType = "3d",
        image = Config.Module.image or "images/power/dvr_coagulis.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_coagulis:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_coagulis:prepareProjectile', source)
            return true
        end
    }
    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_coagulis] Module enregistré avec succès')
end

RegisterNetEvent('dvr_coagulis:broadcastProjectile', function(finalTargetCoords)
    local _source = source
    if not finalTargetCoords then
        return
    end
    TriggerClientEvent('dvr_coagulis:fireProjectile', -1, _source, finalTargetCoords)
end)

RegisterNetEvent('dvr_coagulis:stopBleedServer', function(targetId)
    local _source = source
    if not targetId or targetId <= 0 then
        targetId = _source
    end
    
    TriggerEvent('dvr_sanguiris:stopBleed', targetId)
    
    TriggerClientEvent('dvr_coagulis:stopBleedRemote', -1, targetId)
