            ...
            if type(data) == "table" then
                if currentTime < data.endTime and DoesEntityExist(data.prop) then
                    ...
                    if DoesEntityExist(myPed) then
                        ...
                        if dist < 1.5 then
                            TriggerServerEvent('dvr_coagulis:stopBleedServer', GetPlayerServerId(PlayerId()))
...
RegisterNetEvent('dvr_coagulis:stopBleedRemote', function(targetId)
    local me = GetPlayerServerId(PlayerId())
    if targetId == 0 or targetId == me then
        TriggerEvent('dvr_sanguiris:stopBleed')
        lib.notify({
            title = 'Coagulis',
...
RegisterNetEvent('dvr_coagulis:prepareProjectile', function()
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_coagulis:broadcastProjectile', finalTargetCoords)
...
RegisterNetEvent('dvr_coagulis:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_coagulis:fireProjectile', function(sourceServerId, targetCoords)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
