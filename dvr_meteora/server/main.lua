...
RegisterNetEvent('dvr_meteora:broadcastSignalProjectile', function(targetCoords, spellLevel)
    ...
    TriggerClientEvent('dvr_meteora:spawnSignalProjectile', -1, _source, targetCoords, spellLevel or 1)
end)

-- Event: Apply damage for a single meteor impact (uses centralized damage system)
RegisterNetEvent('dvr_meteora:applyMeteorDamage', function(impactCoords, spellLevel, meteorIndex)
    local _source <const> = source
    ...
    if Config.Debug then
        print(string.format('[dvr_meteora] Meteor %d impact - Level %d - Coords: %.2f, %.2f, %.2f',
            meteorIndex or 0, spellLevel, impactCoords.x, impactCoords.y, impactCoords.z))
    end
end)

--- Register the Meteora module with dvr_power
local function RegisterMeteoraModule()
    ...
    local moduleData <const> = {
        ...
        soundType = "3d",
        image = Config.Module.image or "images/power/dvr_meteora.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_meteora:otherPlayerCasting', -1, source)

            -- Start cast sequence on caster's client
            TriggerClientEvent('dvr_meteora:prepareCast', source, spellLevel)
    ...
    print('[dvr_meteora] Module enregistré avec succès')
end

-- Wait for dvr_power to be ready before registering
...
-- Re-register if dvr_power restarts
