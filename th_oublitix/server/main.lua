...
local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['th_power']:GetSpell(sourceId, 'oublitix')
    end)
...
RegisterNetEvent('th_oublitix:broadcastProjectile', function(finalTargetCoords, targetEntity)
    ...
    TriggerClientEvent('th_oublitix:fireProjectile', -1, _source, finalTargetCoords, targetEntity, spellLevel)
end)

-- Event to apply spell effect on target
RegisterNetEvent('th_oublitix:applySpell', function(targetServerId, spellLevel)
    local _source <const> = source
    ...
    if exports['th_prothea'] and exports['th_prothea'].hasActiveShield and exports['th_prothea']:hasActiveShield(targetServerId) then
        print(string.format('[th_oublitix] Sort annulé par Prothea pour %s', targetServerId))
        return
    end
    ...
    if not targetCoords then
        print(string.format('[th_oublitix] Aucune position trouvée pour le joueur %s (%d minutes)', targetServerId, rollbackMinutes))
    ...
    TriggerClientEvent('th_oublitix:applyEffect', targetServerId, rollbackMinutes, targetCoords)
    
    print(string.format('[th_oublitix] Sort appliqué sur le joueur %s - Rollback de %d minute(s) - Niveau %d', 
...
local function RegisterOublitixModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_oublitix:otherPlayerCasting', -1, source)
            -- Prepare projectile on caster client
            TriggerClientEvent('th_oublitix:prepareProjectile', source, spellLevel)
    ...
    print('[th_oublitix] Module enregistré avec succès')
end

-- Wait for th_power to be ready
CreateThread(function()
