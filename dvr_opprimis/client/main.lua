...
local function attachRopeProp()
    ...
    if not loadModel(ROPE_PROP_MODEL) then
        print('[dvr_opprimis] Erreur: Impossible de charger le modele ' .. ROPE_PROP_MODEL)
        return
    end
    ...
    if not ropeProp or ropeProp == 0 then
        print('[dvr_opprimis] Erreur: Impossible de creer le prop de corde')
        return
    end
...
RegisterNetEvent('dvr_opprimis:onHandcuffed', function(duration, casterId)
    isHandcuffed = true
...
RegisterNetEvent('dvr_opprimis:onReleased', function()
    isHandcuffed = false
    handcuffDuration = 0
    casterServerId = nil
    handcuffEndTime = 0
