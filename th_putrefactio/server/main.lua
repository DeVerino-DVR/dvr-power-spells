...
RegisterNetEvent('th_putrefactio:broadcastProjectile', function(finalTargetCoords)
    ...
    if GetResourceState('th_power') == 'started' then
        clamped = exports['th_power']:ClampSpellCoords(_source, 'putrefactio', coords, nil, false) or coords
    end

    TriggerClientEvent('th_putrefactio:fireProjectile', -1, _source, clamped)
end)
...
local function RegisterPutrefactioModule()
    local moduleData <const> = {
        ...
        soundType = "3d",
        image = "images/power/th_putrefactio.png",
        ...
        onCast = function(hasItem, raycast, source, target)
            ...
            if IsValidTarget(target, source) then
                SetTimeout(1500, function()
                    TriggerClientEvent('th_putrefactio:teleportTarget', target)
                end)
            end
            exports['th_power']:LogSpellCast(data)

            TriggerClientEvent('th_putrefactio:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_putrefactio:prepareProjectile', source)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_putrefactio] Module enregistré avec succès')
end
