...
RegisterNetEvent('th_hiddenis:setHidden', function(hidden)
    ...
    TriggerClientEvent('th_hiddenis:syncHidden', -1, source, hidden)
end)

RegisterNetEvent('th_hiddenis:applyToTarget', function(targetServerId, profile)
    local _source <const> = source
    local target = targetServerId and targetServerId > 0 and targetServerId or _source
    TriggerClientEvent('th_hiddenis:apply', target, profile)
end)
...
AddEventHandler('playerJoining', function()
    ...
    for serverId, _ in pairs(hiddenPlayers) do
        TriggerClientEvent('th_hiddenis:syncHidden', source, serverId, true)
    end
end)
...
AddEventHandler('playerDropped', function()
    ...
    if hiddenPlayers[source] then
        hiddenPlayers[source] = nil
        TriggerClientEvent('th_hiddenis:syncHidden', -1, source, false)
    end
end)
...
local function RegisterHiddenisModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_hiddenis:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_hiddenis:prepareHidden', source, target or 0, profile)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_hiddenis] Module enregistré avec succès')
end
