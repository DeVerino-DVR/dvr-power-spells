...
RegisterNetEvent('th_voidrift:spawnRift', function(targetCoords)
    local _source <const> = source
    if not targetCoords then
        return
    end
    TriggerClientEvent('th_voidrift:createRift', -1, _source, targetCoords)
end)
...
local function RegisterVoidriftModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_voidrift:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_voidrift:prepareRift', source)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_voidrift] Module enregistré avec succès')
end
