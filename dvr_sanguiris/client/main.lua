            ...
            if type(data) == "table" then
                ...
                if currentTime >= data.endTime then
                    if DoesEntityExist(data.prop) then
                        ...
                        TriggerServerEvent('dvr_sanguiris:applyBleed', data.targetId or 0, data.level or 0)
...
RegisterNetEvent('dvr_sanguiris:startBleed', function(tickDamage, tickInterval, maxDuration, fx)
...
RegisterNetEvent('dvr_sanguiris:ragdoll', function(duration)
    if HasProtheaShield() then return end
...
RegisterNetEvent('dvr_sanguiris:stopBleed', function()
    bleedState.loopFxActive = false
    bleedState.active = false
    if bleedState.loopThread then
        bleedState.loopThread = nil
...
RegisterNetEvent('dvr_sanguiris:bleedImpactFx', function(victimId, fx)
...
RegisterNetEvent('dvr_sanguiris:bleedRecurrentFx', function(victimId, fx)
    local me = GetPlayerServerId(PlayerId())
    if victimId == me then return end
    local player = GetPlayerFromServerId(victimId)
    if player == -1 then return end
...
RegisterNetEvent('dvr_sanguiris:stopBleedFx', function(victimId)
...
RegisterNetEvent('dvr_sanguiris:applyBleedClient', function(level)
    if HasProtheaShield() then return end
    TriggerServerEvent('dvr_sanguiris:applyBleed', GetPlayerServerId(PlayerId()), level or 0)
end)

RegisterNetEvent('dvr_sanguiris:prepareProjectile', function(targetId, level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_sanguiris:broadcastProjectile', finalTargetCoords, targetId or 0, level or 0)
        Wait(cleanupDelay)
        RemoveWandParticles(casterPed)
    end)
end)

RegisterNetEvent('dvr_sanguiris:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_sanguiris:spawnProjectile', function(sourceServerId, targetCoords, targetId, level)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then return end
    local casterPed = GetPlayerPed(casterPlayer)
    if not DoesEntityExist(casterPed) then return end
