            ...
            if type(data) == "table" then
                ...
                if currentTime >= data.endTime then
                    if DoesEntityExist(data.prop) then
                        ...
                        TriggerServerEvent('dvr_ragdolo:ragdollTarget', data.targetId or 0, data.level or 0)
...
RegisterNetEvent('dvr_ragdolo:applyRagdoll', function(level)
...
RegisterNetEvent('dvr_ragdolo:prepareProjectile', function(targetId, level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_ragdolo:broadcastProjectile', finalTargetCoords, targetId or 0, level or 0)
...
RegisterNetEvent('dvr_ragdolo:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_ragdolo:spawnProjectile', function(sourceServerId, targetCoords, targetId, level)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
