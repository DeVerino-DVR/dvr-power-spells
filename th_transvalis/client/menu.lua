...
local function LoadTransvalisConfig()
    if not exports['th_power'] then return end

    local hasGetSpellConfig = pcall(function() return exports['th_power'].GetSpellConfig end)
    if not hasGetSpellConfig then return end

    local loadedConfig = exports['th_power']:GetSpellConfig('transvalis')
...
local function SaveTransvalisConfig(skipCallback)
    if not exports['th_power'] then return false end

    local hasSaveSpellConfig = pcall(function() return exports['th_power'].SaveSpellConfig end)
    ...
    return exports['th_power']:SaveSpellConfig('transvalis', transvalisConfig, skipCallback)
end
...
local function LoadSavedPosition(slot)
    ...
    if exports['th_transvalis'] and exports['th_transvalis'].SaveReturnPosition then
        exports['th_transvalis']:SaveReturnPosition()
    else
        TriggerEvent('th_transvalis:saveReturnPosition')
    end
    ...
    CreateThread(function()
        ...
        if transvalisConfig.bringPlayers then
            ...
            if #targets > 0 and exports['th_transvalis'] and exports['th_transvalis'].SetTeleportedPlayers then
                exports['th_transvalis']:SetTeleportedPlayers(targets)
            end
        end
        ...
        if #targets > 0 then
            ...
            TriggerServerEvent('th_transvalis:teleportMultiplePlayers', targets, destinationCoords)
...
local function CreateTransvalisConfigMenu()
    ...
    lib.registerMenu({
        ...
        elseif args.action == 'return_players' then
            if exports['th_transvalis'] and exports['th_transvalis'].ReturnTeleportedPlayers then
                exports['th_transvalis']:ReturnTeleportedPlayers()
                lib.hideMenu()
            else
                lib.notify({
                    title = 'Erreur',
...
CreateThread(function()
    while true do
        if exports['th_power'] then
            local hasGetSpellConfig = pcall(function() return exports['th_power'].GetSpellConfig end)
            local hasRegisterCallback = pcall(function() return exports['th_power'].RegisterSpellConfigCallback end)
            local hasRegisterConfigurable = pcall(function() return exports['th_power'].RegisterConfigurableSpell end)
    ...
    while not exports['th_power'].getPlayerSpells or #exports['th_power']:getPlayerSpells() == 0 do
        Wait(100)
    end
    ...
    while not spellReady and attempts < 50 do
        if exports['th_power'].GetSpell then
            local ok, _, level = pcall(function()
                return exports['th_power']:GetSpell('transvalis')
            end)
            if ok and level and tonumber(level) then
    ...
    exports['th_power']:RegisterSpellConfigCallback('transvalis', function(config)
