...
RegisterNetEvent('dvr_ravivio:broadcastProjectile', function(finalTargetCoords)
    ...
    TriggerClientEvent('dvr_ravivio:fireProjectile', -1, _source, finalTargetCoords, targetPlayer)
end)

RegisterNetEvent('dvr_ravivio:projectileHit', function(targetPlayer)
    local _source <const> = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    TriggerClientEvent('dvr_ravivio:playReviveEffect', -1, targetPlayer)
...
local function RegisterRavivioModule()
    local moduleData <const> = {
        ...
        soundType = "3d",
        image = Config.Module.image or "images/power/dvr_ravivio.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_ravivio:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_ravivio:prepareCast', source)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_ravivio] Module enregistré avec succès')
end
