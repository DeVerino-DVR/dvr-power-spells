...
local function handcuffPlayer(playerId, duration, casterServerId)
    ...
    TriggerClientEvent('th_opprimis:onHandcuffed', playerId, duration, casterServerId)
    ...
    print(string.format('[th_opprimis] HANDCUFF: %s (ID: %d) menotte pour %d secondes par ID: %d',
...
function releasePlayer(playerId)
    ...
    TriggerClientEvent('th_opprimis:onReleased', playerId)

    if Config.Messages and Config.Messages.released then
        lib.notify(playerId, Config.Messages.released)
    end

    print(string.format('[th_opprimis] RELEASE: Player %d released from handcuffs', playerId))
end
...
local function registerModule()
    ...
    local moduleData <const> = {
        ...
        onCast = function(hasWand, _, source, target, spellLevel)
            ...
            if exports['th_prothea'] and exports['th_prothea'].hasActiveShield and exports['th_prothea']:hasActiveShield(target) then
                print(string.format('[th_opprimis] Sort annule par Prothea pour %s', target))
                return false
            end
    ...
    print('[th_opprimis] Module enregistre avec succes')
end
