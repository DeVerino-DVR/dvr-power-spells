...
RegisterNetEvent('th_cyclone:trigger', function(targetCoords)
    ...
    TriggerClientEvent('th_cyclone:createCyclone', -1, _source, targetCoords, level)
end)
...
local function RegisterCycloneModule()
    local moduleData <const> = {
        ...
        soundType = "3d",
        image = Config.Module.image or "images/power/th_cyclone.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_cyclone:otherPlayerCasting', -1, source, clamped)
            TriggerClientEvent('th_cyclone:prepare', source, clamped)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_cyclone] Module enregistré avec succès')
end
