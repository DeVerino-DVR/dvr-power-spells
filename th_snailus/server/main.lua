...
local function SyncPlayerState(target, serverId, enabled, durationMs, speedMultiplier)
    TriggerClientEvent('th_snailus:sync', target, serverId, enabled, durationMs, speedMultiplier)
end

RegisterNetEvent('th_snailus:requestSync', function()
...
RegisterNetEvent('th_snailus:timeExpired', function(serverId)
...
local function RegisterSnailusModule()
    ...
    local moduleData <const> = {
        ...
        isBasic = false,
        selfCast = false,
        image = Config.Module.image or 'images/power/th_snailus.png',
    ...
    print('[th_snailus] Module enregistré avec succès')
end
