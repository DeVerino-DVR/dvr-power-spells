...
local function PlayPukeEffect(targetServerId)
    ...
    StopPukeForPed(ped)
    print(('[dvr_degueulis] start puke for ped %s'):format(ped))
    ...
    if effectDuration and effectDuration > 0 then
        SetTimeout(effectDuration, function()
            StopPukeForPed(ped)
            print(('[dvr_degueulis] stop puke for ped %s (timeout %dms)'):format(ped, effectDuration))
        end)    
    end
end

RegisterNetEvent('dvr_degueulis:playPuke', function(targetServerId)
    PlayPukeEffect(targetServerId)
end)

RegisterNetEvent('dvr_degueulis:fireProjectile', function(sourceServerId, targetCoords, targetServerId)
            ...
            if type(data) == "table" and DoesEntityExist(propId) then
                if now < data.endTime then
                    ...
                    if data.targetServerId then
                        TriggerServerEvent('dvr_degueulis:onImpact', data.targetServerId)
                    end
