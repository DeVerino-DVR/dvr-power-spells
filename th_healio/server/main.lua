...
local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['th_power']:GetSpell(sourceId, 'healio')
    end)
...
RegisterNetEvent('th_healio:broadcastProjectile', function(finalTargetCoords)
    ...
    TriggerClientEvent('th_healio:fireProjectile', -1, _source, finalTargetCoords, spellLevel)
end)
...
local function RegisterHealioModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_healio:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_healio:prepareProjectile', source)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_healio] Module enregistré avec succès')
end
