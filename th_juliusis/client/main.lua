            ...
            if type(data) == "table" then
                ...
                if currentTime >= data.endTime then
                    if DoesEntityExist(data.prop) then
                        ...
                        TriggerServerEvent('th_juliusis:ragdollTarget', data.targetId or 0, data.level or 0)
...
RegisterNetEvent('th_juliusis:playSound', function(sourceServerId, casterCoords)
...
RegisterNetEvent('th_juliusis:applyRagdoll', function(level)
...
RegisterNetEvent('th_juliusis:prepareProjectile', function(targetId, level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('th_juliusis:broadcastProjectile', finalTargetCoords, targetId or 0, level or 0)
...
RegisterNetEvent('th_juliusis:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('th_juliusis:spawnProjectile', function(sourceServerId, targetCoords, targetId, level)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
