...
RegisterNetEvent('th_sufferis:broadcastProjectile', function(targetCoords, targetId, level)
    ...
    TriggerClientEvent('th_sufferis:spawnProjectile', -1, _source, targetCoords, targetId or 0, level or 0)
end)

RegisterNetEvent('th_sufferis:ragdollTarget', function(targetId, level)
    if not targetId or targetId <= 0 then
        return
    end

    TriggerClientEvent('th_sufferis:applyRagdoll', targetId, level or 0)
end)
...
local function RegisterSufferisModule()
    local moduleData <const> = {
        ...
        sound = '',
        soundType = "3d",
        image = Config.Module.image or "images/power/th_sufferis.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_sufferis:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_sufferis:prepareProjectile', source, target or 0, level or 0)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_sufferis] Module enregistré avec succès')
end
