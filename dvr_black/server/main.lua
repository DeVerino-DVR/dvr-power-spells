...
local function SyncPlayerState(target, serverId, enabled, profile)
    SetTimeout(3000, function()
        TriggerClientEvent('dvr_black:sync', target, serverId, enabled, profile)
    end)
end

local function TriggerTransitionFx(serverId, profile, enabled)
    SetTimeout(3000, function()
        TriggerClientEvent('dvr_black:transitionFx', -1, serverId, profile, enabled == true)
    end)
end

RegisterNetEvent('dvr_black:requestSync', function()
...
local function RegisterBlackModule()
    local moduleData <const> = {
        ...
        isBasic = false,
        selfCast = true,
        image = Config.Module.image or 'images/power/dvr_black.png',
    ...
    print('[dvr_black] Module enregistré avec succès')
end
