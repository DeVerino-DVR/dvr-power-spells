...
RegisterNetEvent('th_basic:prepareProjectile', function()
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('th_basic:broadcastProjectile', finalTargetCoords)
...
RegisterNetEvent('th_basic:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('th_basic:fireProjectile', function(sourceServerId, targetCoords)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then 
        return 
    end
...
RegisterNetEvent('th_basic:removeHealth', function(level)
