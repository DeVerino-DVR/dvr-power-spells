...
local function EnsurePtfxAsset(asset)
    ...
    -- On considère l'asset chargé après le loop (comme th_transvalis)
...
local function PlayTransitionFx(serverId, ped, profile)
    ...
    if not handle or handle == 0 then
        if Config and Config.Debug then
            print(('[th_black] PTFX FAIL asset=%s effect=%s attach=%s bone=%s scale=%s'):format(
                tostring(asset),
                tostring(effect),
                tostring(attachMode),
                tostring(cfg.bone),
...
RegisterNetEvent('th_black:sync', function(serverId, enabled, profile)
    SetBlackState(serverId, enabled == true, profile)
end)

RegisterNetEvent('th_black:transitionFx', function(serverId, profile, enabled)
    local ped = GetPedByServerId(serverId)
    if not ped then
        return
    end
...
CreateThread(function()
    Wait(2500)
    TriggerServerEvent('th_black:requestSync')
