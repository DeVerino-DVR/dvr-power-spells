...
- Each spell lives in its own `th_<spell>` subfolder (e.g., `th_prothea/`) under this resource root. Every module follows the `client/`, `server/`, `config.lua`, `fxmanifest.lua` pattern so FiveM can `ensure th_<spell>`. `client/main.lua` drives visuals and effects, while `server/main.lua` enforces gameplay state and networking.
...
- `ensure th_<spell>` (e.g., `ensure th_prothea`) boots the individual resource for live testing without restarting the entire server.
- `stop th_<spell>` gracefully unloads a spell before redeployment or memory leak checks.
...
- Confirm spell behavior through live tests, using `ensure th_<spell>` and the admin `lspell`/`uspell` helpers to trigger scenarios.
