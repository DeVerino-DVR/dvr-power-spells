...
local function SendCastResult(target, status, data)
    ...
    TriggerClientEvent('th_prothea:castResult', target, status, data or {})
end
        ...
        spells = {
            {
                ...
                selfCast = true,
                professor = true,
                image = 'images/power/th_prothea.png',
                keys = Config and Config.Key or nil
            }
        }
    }, 0)
    print('[th_prothea] Module enregistré avec succès')
end
    ...
    while true do
        ...
        for playerId, shield in pairs(playerShields or {}) do
            if shield.endTime <= currentTime then
                ...
                if source then
                    TriggerClientEvent('th_prothea:removeShield', -1, source)
                end
            end
        end
    end
...
RegisterNetEvent('th_prothea:castShield', function()
    ...
    TriggerClientEvent('th_prothea:applyShield', -1, source, durationMs, blockPercentage, applyGodmode, useProps, spellLevel)
end)
...
AddEventHandler('playerDropped', function()
    ...
    TriggerClientEvent('th_prothea:removeShield', -1, sourceId)
end)

RegisterNetEvent('th_prothea:forceDeleteEntity', function(netId)
    ...
    if entity and entity ~= 0 and DoesEntityExist(entity) then
        ...
        if DoesEntityExist(entity) then
            ...
            if owner and owner > 0 then
                TriggerClientEvent('th_prothea:forceDeleteEntityClient', owner, netId)
            end
            TriggerClientEvent('th_prothea:forceDeleteEntityClient', -1, netId)
            return
        end
