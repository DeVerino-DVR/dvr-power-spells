...
RegisterNetEvent('dvr_petrosa:broadcastProjectile', function(targetCoords, spellLevel)
    ...
    TriggerClientEvent('dvr_petrosa:fireProjectile', -1, _source, targetCoords, spellLevel or 1)
end)

--- Event: Apply rock impact damage
RegisterNetEvent('dvr_petrosa:applyDamage', function(impactCoords, spellLevel)
    local _source <const> = source
    ...
    if Config.Debug then
        print(string.format('[dvr_petrosa] Rock impact - Level %d - Coords: %.2f, %.2f, %.2f',
            spellLevel, impactCoords.x, impactCoords.y, impactCoords.z))
    end
end)

--- Register the Petrosa module with dvr_power
local function RegisterPetrosaModule()
    ...
    local moduleData <const> = {
        ...
        soundType = "3d",
        image = Config.Module.image or "images/power/dvr_petrosa.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_petrosa:otherPlayerCasting', -1, source)

            -- Start cast sequence on caster's client
            TriggerClientEvent('dvr_petrosa:prepareProjectile', source, spellLevel)
    ...
    print('[dvr_petrosa] Module enregistré avec succès')
end
