...
RegisterNetEvent('th_accyra:prepareProjectile', function(spellLevel)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('th_accyra:broadcastProjectile', finalTargetCoords)
...
RegisterNetEvent('th_accyra:otherPlayerCasting', function(sourceServerId, spellLevel)
...
RegisterNetEvent('th_accyra:fireProjectile', function(sourceServerId, targetCoords, spellLevel)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then 
        return 
    end
