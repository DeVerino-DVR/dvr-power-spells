...
RegisterNetEvent('th_accyra:broadcastProjectile', function(finalTargetCoords)
    ...
    TriggerClientEvent('th_accyra:fireProjectile', -1, _source, finalTargetCoords, level)
...
local function RegisterAccioModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, spellLevel)
            ...
            TriggerClientEvent('th_accyra:otherPlayerCasting', -1, source, level)
            TriggerClientEvent('th_accyra:prepareProjectile', source, level)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_accyra] Module enregistré avec succès')
end
