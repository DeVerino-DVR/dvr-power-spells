...
local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['th_power']:GetSpell(sourceId, 'firedrix')
    end)
        ...
        for effectId, effect in pairs(activeEffects) do
            ...
            for _, player in ipairs(players) do
                ...
                local ok, applied = pcall(function()
                    return exports['th_power']:DealDamage(playerId, effect.damage, effect.caster or 0, 'firedrix')
                end)
                ...
                if ok and applied then
                    ...
                    if (now - lastNotify) >= 2000 then
                        TriggerClientEvent('th_incendrix:setPlayerOnFire', playerId, 3000)
                        effect.notified[playerId] = now
                    end
                elseif not ok and Config.Debug then
                    print(('[th_firedrix] Damage failed for %s: %s'):format(playerId or 'nil', applied or 'unknown'))
                end
...
RegisterNetEvent('th_firedrix:broadcastProjectile', function(targetCoords, radius, duration)
    local source = source
    ...
    TriggerClientEvent('th_firedrix:fireProjectile', -1, source, targetCoords, pending.radius, pending.duration)
...
local function RegisterIncendioModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_firedrix:prepareCast', source, settings.radius, settings.duration)
            TriggerClientEvent('th_firedrix:otherPlayerCasting', -1, source)
    ...
    print('[th_firedrix] Module enregistré avec succès')
end
