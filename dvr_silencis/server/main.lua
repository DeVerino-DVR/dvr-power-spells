...
local function mutePlayer(playerId, duration, reason)
    ...
    if not exports['pma-voice'] or not exports['pma-voice'].isValidPlayer then
        print('[dvr_silencis] ERROR: pma-voice export not available')
        return false
    end
    
    if not exports['pma-voice']:isValidPlayer(playerId) then
        print(string.format('[dvr_silencis] ERROR: Player %d is not valid', playerId))
        return false
    end
    ...
    print(string.format('[dvr_silencis] MUTE: %s (ID: %d) muté pour %d secondes - Raison: %s', 
...
local function registerModule()
    ...
    local moduleData <const> = {
        ...
        onCast = function(hasWand, _, source, target, spellLevel)
            ...
            if exports['dvr_prothea'] and exports['dvr_prothea'].hasActiveShield and exports['dvr_prothea']:hasActiveShield(target) then
                print(string.format('[dvr_silencis] Sort annulé par Prothea pour %s', target))
                return false
            end
    ...
    print('[dvr_silencis] Module enregistré avec succès')
end
