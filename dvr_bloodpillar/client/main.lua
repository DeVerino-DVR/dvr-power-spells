            ...
            for levitationId, levData in pairs(activeLevitations) do
                if DoesEntityExist(levData.ped) then
                    ...
                    if currentTime >= levData.endTime then
                        ...
                        if isLocalPlayer then
                            TriggerServerEvent('dvr_bloodpillar:levitationExpired', levitationId)
                        end
                    end
                else
                    if levData.auraFx then
...
RegisterNetEvent('dvr_bloodpillar:startLevitation', function(targetServerId, casterServerId, duration, height, level)
...
RegisterNetEvent('dvr_bloodpillar:playSecondAnim', function()
...
RegisterNetEvent('dvr_bloodpillar:otherPlayerSecondAnim', function(sourceServerId)
...
RegisterNetEvent('dvr_bloodpillar:expelTarget', function(casterServerId, targetServerId, level)
    ...
    CreateThread(function()
        ...
        if isCaster then
            TriggerServerEvent('dvr_bloodpillar:applyExpulsionDamage', impactCoords, level)
        end
    end)
end)
