                ...
                if currentTime >= data.endTime then
                    ...
                    if DoesEntityExist(data.prop) then
                        ...
                        if data.sourceServerId == myServerId then
                            TriggerServerEvent('th_ignifera:applyExplosionDamage', propCoords, level)
                        end
...
RegisterNetEvent('th_ignifera:prepareProjectile', function(spellLevel)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('th_ignifera:broadcastProjectile', finalTargetCoords, spellLevel)
...
RegisterNetEvent('th_ignifera:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('th_ignifera:fireProjectile', function(sourceServerId, targetCoords, spellLevel)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then 
        return 
    end
