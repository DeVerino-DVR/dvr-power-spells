...
local function PlayImpactSound(coords)
    ...
    -- id = ('dvr_firedrix_impact_%s'):format(GetGameTimer()),
    -- url = soundCfg.url,
...
RegisterNetEvent('dvr_firedrix:prepareCast', function(radius, duration)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_firedrix:broadcastProjectile', finalTargetCoords, radius, duration)
...
RegisterNetEvent('dvr_firedrix:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_firedrix:fireProjectile', function(sourceServerId, targetCoords, radius, duration)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
...
RegisterNetEvent('dvr_incendrix:setPlayerOnFire', function(duration)
