...
RegisterNetEvent('dvr_aquamens:doTeleport', function(startCoords, targetCoords)
    ...
    TriggerClientEvent('dvr_aquamens:runTeleport', -1, _source, startCoords, targetCoords)
end)
...
local function RegisterAquamensModule()
    local moduleData <const> = {
        ...
        sound = '',
        soundType = "3d",
        image = Config.Module.image or "images/power/dvr_aquamens.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_aquamens:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_aquamens:prepareTeleport', source)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_aquamens] Module enregistré avec succès')
end
