                ...
                if currentTime >= data.endTime then
                    if DoesEntityExist(data.prop) then
                        ...
                        if isLocalCaster then
                            TriggerServerEvent('dvr_thunder:applyLightningDamage', propCoords, data.level or 0)
                        end
                        ...
                        if isLocalCaster and isLastShot then
                            TriggerServerEvent('dvr_thunder:ragdollTarget', data.targetId or 0, data.level or 0)
...
RegisterNetEvent('dvr_thunder:applyRagdoll', function(level)
...
local function PlayLaunchSound(coords)
    ...
    -- id = ('dvr_thunder_launch_%s'):format(GetGameTimer()),
...
local function PlayCastSound(coords)
    ...
    -- REPLACE WITH YOUR SOUND SYSTEM
    -- exports['lo_audio']:playSound({
    -- id = ('dvr_thunder_cast_%s'):format(GetGameTimer()),
...
RegisterNetEvent('dvr_thunder:prepareProjectile', function(targetId, level)
    ...
    CreateThread(function()
        ...
        for index, delayMs in ipairs(shotTimings) do
            ...
            TriggerServerEvent('dvr_thunder:broadcastProjectile', finalTargetCoords, targetId or 0, level or 0, index, #shotTimings)
...
RegisterNetEvent('dvr_thunder:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_thunder:spawnProjectile', function(sourceServerId, targetCoords, targetId, level, shotIndex, shotTotal)
    local casterPlayer<const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
