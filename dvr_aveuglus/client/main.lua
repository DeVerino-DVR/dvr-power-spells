...
RegisterNetEvent('dvr_aveuglus:sync', function(serverId, enabled, durationMs)
        ...
        if isLocalBlinded then
            ...
            if localBlindEndTime and localBlindEndTime ~= -1 then
                ...
                if now >= localBlindEndTime then
                    local localServerId = GetPlayerServerId(PlayerId())
                    TriggerServerEvent('dvr_aveuglus:timeExpired', localServerId)
                    RemoveBlindEffect()
                end
            end
        end
...
CreateThread(function()
    Wait(2500)
    TriggerServerEvent('dvr_aveuglus:requestSync')
end)
