...
local function PlayImpactSound(coords)
    ...
    -- id = ('th_pyroth_impact_%s'):format(GetGameTimer()),
    -- url = soundCfg.url,
...
local function SpawnFlameZone(coords, level, sourceServerId)
    ...
    if sourceServerId == myServerId then
        TriggerServerEvent('th_pyroth:applyFlameDamage', coords, level, duration)
    end
end
...
RegisterNetEvent('th_pyroth:prepareCast', function(level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('th_pyroth:broadcastProjectile', finalTargetCoords, level or 1)
...
RegisterNetEvent('th_pyroth:otherPlayerCasting', function(sourceServerId)
    local myServerId = GetPlayerServerId(PlayerId())
...
RegisterNetEvent('th_pyroth:fireProjectile', function(sourceServerId, targetCoords, level)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
...
RegisterNetEvent('th_pyroth:setPlayerOnFire', function(duration)
