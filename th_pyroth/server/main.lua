...
local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['th_power']:GetSpell(sourceId, 'pyroth')
    end)
...
CreateThread(function()
    while true do
        ...
        for zoneId, zone in pairs(activeFlameZones) do
            ...
            local players = exports['th_power']:GetPlayersInRadius(
                zone.coords, 
                zone.radius, 
                zone.caster
            )
            ...
            if players then
                for _, playerData in ipairs(players) do
                    ...
                    if playerId then
                        ...
                        TriggerClientEvent('th_pyroth:setPlayerOnFire', playerId, zone.burnDuration or 3000)
                    end
                end
            end
...
RegisterNetEvent('th_pyroth:broadcastProjectile', function(targetCoords, level)
    ...
    TriggerClientEvent('th_pyroth:fireProjectile', -1, _source, targetCoords, spellLevel)
end)

-- Applique les dégâts de la zone de flammes via le damage_system de th_power
RegisterNetEvent('th_pyroth:applyFlameDamage', function(flameCoords, level, duration)
    local _source = source
    ...
    local players = exports['th_power']:GetPlayersInRadius(coords, settings.radius, _source)
    
    if players then
        for _, playerData in ipairs(players) do
            local playerId = playerData[1] or playerData.playerId
            if playerId then
                TriggerClientEvent('th_pyroth:setPlayerOnFire', playerId, settings.burnDuration or 3000)
            end
        end
    end
...
local function RegisterPyrothModule()
    ...
    local moduleData = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_pyroth:prepareCast', source, spellLevel)
            TriggerClientEvent('th_pyroth:otherPlayerCasting', -1, source)
    ...
    print('[th_pyroth] Module Pyroth enregistré avec succès')
end

-- Attendre que th_power soit démarré
CreateThread(function()
...
-- Re-enregistrer si th_power redémarre
