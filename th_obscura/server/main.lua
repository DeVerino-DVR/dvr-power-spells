...
RegisterNetEvent('th_obscura:startBlackout', function(level)
    local _source <const> = source
    local settings <const> = BuildBlackoutSettings(level)
    TriggerClientEvent('th_obscura:applyBlackout', -1, _source, settings)
end)
...
local function RegisterObscuraModule()
    local moduleData <const> = {
        ...
        sound = '',
        soundType = "3d",
        image = Config.Module.image or "images/power/th_obscura.png",
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_obscura:otherPlayerCasting', -1, source)
            local lvl <const> = math.max(math.floor(tonumber(level) or 1), 1)
            TriggerClientEvent('th_obscura:prepareBlackout', source, lvl)
    ...
    print('[th_obscura] Module enregistré avec succès')
end
