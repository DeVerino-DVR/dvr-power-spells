...
local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['th_power']:GetSpell(sourceId, 'staturion')
    end)
...
RegisterNetEvent('th_staturion:broadcastProjectile', function(finalTargetCoords, targetEntity)
    ...
    TriggerClientEvent('th_staturion:fireProjectile', -1, _source, finalTargetCoords, targetEntity, spellLevel, duration)
end)

RegisterNetEvent('th_staturion:petrifyPlayer', function(targetServerId)
    local _source <const> = source
    ...
    TriggerClientEvent('th_staturion:applyPetrify', targetServerId, duration, spellLevel)
    
    SetTimeout(duration, function()
        TriggerClientEvent('th_staturion:removePetrify', targetServerId)
    end)
...
local function RegisterPetrificusModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_staturion:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_staturion:prepareProjectile', source, spellLevel)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_staturion] Module enregistré avec succès')
end
