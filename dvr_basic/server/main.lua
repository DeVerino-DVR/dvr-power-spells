...
RegisterNetEvent('dvr_basic:broadcastProjectile', function(finalTargetCoords)
    local _source <const> = source
    if not finalTargetCoords then
        return
    end
    
    TriggerClientEvent('dvr_basic:fireProjectile', -1, _source, finalTargetCoords)
end)
    ...
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            if targetId and raycast and raycast.hit then
                TriggerClientEvent('dvr_basic:removeHealth', targetId, level)
            end
            ...
            TriggerClientEvent('dvr_basic:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_basic:prepareProjectile', source)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_basic] Module enregistré avec succès')
end
