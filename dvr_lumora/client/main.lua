...
local function FireLumoraProjectile()
    ...
    TriggerServerEvent('dvr_lumora:fireProjectile', startCoords, targetCoords)
end

RegisterNetEvent('dvr_lumora:client:fireProjectile', function(casterServerId, startCoords, targetCoords)
...
lib.addKeybind({
    ...
    onPressed = function(self)
        ...
        if lumosState and lumosState.active then
            TriggerServerEvent('dvr_lumora:removeLight')
        else
            TriggerServerEvent('dvr_lumora:toggleLight')
        end
    end
})
...
RegisterNetEvent('dvr_power:castSpellByKey', function(spellId)
