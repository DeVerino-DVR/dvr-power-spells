        ...
        spells = {
            {
                ...
                selfCast = true,
                professor = true,
                image = 'images/power/dvr_lumora.png',
                keys = DEFAULT_KEY
            }
        }
    }, 0)
    print('[dvr_lumora] Module enregistré avec succès')
end
...
RegisterNetEvent('dvr_lumora:toggleLight', function()
...
RegisterNetEvent('dvr_lumora:removeLight', function()
    local source <const> = source
    Player(source).state:set('lumos', nil, true)
end)

RegisterNetEvent('dvr_lumora:fireProjectile', function(startCoords, targetCoords)
    local source <const> = source
    TriggerClientEvent('dvr_lumora:client:fireProjectile', -1, source, startCoords, targetCoords)
end)
