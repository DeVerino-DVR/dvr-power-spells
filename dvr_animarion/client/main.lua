                            ...
                            if data.targetEntity and DoesEntityExist(data.targetEntity) then
                                if IsEntityAPed(data.targetEntity) then
                                    ...
                                    if IsPedAPlayer(data.targetEntity) then
                                        ...
                                        TriggerServerEvent('dvr_animarion:transformPlayer', targetServerId)
                                    else
                                        local randomAnimal = Config.Animagus.animals[math.random(#Config.Animagus.animals)]
                                        TriggerEvent('dvr_animarion:transformPed', data.targetEntity, randomAnimal, transformDuration)
                                    end
                                end
                            end
...
RegisterNetEvent('dvr_animarion:prepareProjectile', function()
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_animarion:broadcastProjectile', finalTargetCoords, targetEntity)
...
RegisterNetEvent('dvr_animarion:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_animarion:fireProjectile', function(sourceServerId, targetCoords, targetEntity, level, duration)
    local casterPlayer = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then 
        return 
    end
...
RegisterNetEvent('dvr_animarion:applyTransform', function(animalModel, duration)
    ...
    SetTimeout(transformDuration, function()
        TriggerServerEvent('dvr_animarion:revertTransform')
    end)
end)

RegisterNetEvent('dvr_animarion:revertToHuman', function()
...
RegisterNetEvent('dvr_animarion:transformPed', function(pedEntity, animalModel, duration)
