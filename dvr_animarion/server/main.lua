...
local function GetCasterLevel(sourceId)
    local ok, hasSpell, level = pcall(function()
        return exports['dvr_power']:GetSpell(sourceId, 'animarion')
    end)
...
RegisterNetEvent('dvr_animarion:broadcastProjectile', function(finalTargetCoords, targetEntity)
    ...
    TriggerClientEvent('dvr_animarion:fireProjectile', -1, _source, finalTargetCoords, targetEntity, spellLevel, duration)
end)

RegisterNetEvent('dvr_animarion:transformPlayer', function(targetServerId)
    local _source <const> = source
    ...
    if targetServerId and targetServerId ~= -1 then
        TriggerClientEvent('dvr_animarion:applyTransform', targetServerId, randomAnimal, duration, spellLevel)
        
        SetTimeout(duration, function()
            TriggerClientEvent('dvr_animarion:revertToHuman', targetServerId)
        end)
...
RegisterNetEvent('dvr_animarion:revertTransform', function()
    local _source <const> = source
    
    TriggerClientEvent('dvr_animarion:revertToHuman', _source)
end)
...
local function RegisterAnimagusModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_animarion:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_animarion:prepareProjectile', source, spellLevel)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_animarion] Module enregistré avec succès')
end
