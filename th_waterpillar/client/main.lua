...
local function TriggerWaterPillarDamage(coords, level, sourceServerId)
    ...
    if sourceServerId == myServerId then
        TriggerServerEvent('th_waterpillar:applyPillarDamage', coords, level)
    end
...
RegisterNetEvent('th_waterpillar:preparePillar', function(level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('th_waterpillar:broadcastProjectile', finalTargetCoords, level or 0)
...
RegisterNetEvent('th_waterpillar:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('th_waterpillar:spawnPillar', function(sourceServerId, targetCoords, level)
    local casterPlayer<const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
