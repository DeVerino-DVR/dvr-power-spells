...
RegisterNetEvent('th_waterpillar:broadcastProjectile', function(targetCoords, level)
    ...
    TriggerClientEvent('th_waterpillar:spawnPillar', -1, _source, targetCoords, level or 0)
end)

RegisterNetEvent('th_waterpillar:applyPillarDamage', function(pillarCoords, spellLevel)
    local _source <const> = source
...
local function RegisterWaterpillarModule()
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('th_waterpillar:otherPlayerCasting', -1, source)
            TriggerClientEvent('th_waterpillar:preparePillar', source, level or 0)
            return true
        end
    }

    exports['th_power']:registerModule(moduleData, 0)
    print('[th_waterpillar] Module enregistré avec succès')
end
