...
RegisterNetEvent('dvr_obscura:prepareBlackout', function(level)
    local ped = cache.ped
    PlayWandFx(ped)
    ...
    CreateThread(function()
        Wait(4000)
        TriggerServerEvent('dvr_obscura:startBlackout', lvl)
        Wait(800)
        StopWandFx(ped)
    end)
end)

RegisterNetEvent('dvr_obscura:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_obscura:applyBlackout', function(sourceServerId, settings)
