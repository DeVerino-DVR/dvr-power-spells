...
local function TriggerLevelExplosion(coords, level, sourceServerId)
    ...
    if sourceServerId == myServerId then
        TriggerServerEvent('dvr_firepillar:applyPillarDamage', coords, level)
    end
...
RegisterNetEvent('dvr_firepillar:preparePillar', function(level)
    ...
    CreateThread(function()
        ...
        TriggerServerEvent('dvr_firepillar:broadcastPillar', finalTargetCoords, level or 0)
...
RegisterNetEvent('dvr_firepillar:otherPlayerCasting', function(sourceServerId)
...
RegisterNetEvent('dvr_firepillar:spawnPillar', function(sourceServerId, targetCoords, level)
    local casterPlayer <const> = GetPlayerFromServerId(sourceServerId)
    if casterPlayer == -1 then
        return
    end
