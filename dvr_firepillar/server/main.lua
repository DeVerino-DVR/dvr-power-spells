...
RegisterNetEvent('dvr_firepillar:broadcastPillar', function(targetCoords, level)
    ...
    TriggerClientEvent('dvr_firepillar:spawnPillar', -1, _source, targetCoords, level or 0)
end)

RegisterNetEvent('dvr_firepillar:applyPillarDamage', function(pillarCoords, spellLevel)
    local _source <const> = source
...
local function RegisterFirepillarModule()
    ...
    local moduleData <const> = {
        ...
        onCast = function(hasItem, raycast, source, target, level)
            ...
            TriggerClientEvent('dvr_firepillar:otherPlayerCasting', -1, source)
            TriggerClientEvent('dvr_firepillar:preparePillar', source, level or 0)
            return true
        end
    }

    exports['dvr_power']:registerModule(moduleData, 0)
    print('[dvr_firepillar] Module enregistré avec succès')
end
